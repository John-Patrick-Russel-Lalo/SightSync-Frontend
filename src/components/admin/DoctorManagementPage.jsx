...
import { useAuth } from "../../context/AuthContext"; // Adjust import path relative to your file structure
import {
  UserCheck,
  Plus,
  Edit,
...
export default function DoctorManagementPage() {
  ...
  // Construct endpoints dynamic to your environment base URL
  ...
  const [activeTab, setActiveTab] = useState("profile"); // 'profile' or 'schedules'
  ...
  const fetchDoctorUsers = async () => {
    try {
      ...
      const res = await fetch(`${API_USERS_URL}?role=doctor`, {
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      });
      ...
      const availableUsers = (result.data || result || []).filter(
